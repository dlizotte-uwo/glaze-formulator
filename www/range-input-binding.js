...
$.extend(rangeInputBinding, {
  ...
  subscribe: function(el, callback) {
    $(el).on('keyup.rangeInputBinding input.rangeInputBinding', function(event) {
      callback(true);
      // When called with true, it will use the rate policy,
      // which in this case is to debounce at 500ms.
    });
    $(el).on('change.rangeInputBinding', function(event) {
      callback(false);
  ...
  // Messages sent by updateRangeInput() are received by this function.
...
Shiny.inputBindings.register(rangeInputBinding, 'shiny.rangeInput');
